...
**Website snapshots** (`website/pages-*.json`): 9 auto-generated JSON files covering every published page on social.plus — marketing, use cases, blog, glossary, answers, customer stories, release notes, product updates, and webinars. Regenerated by a Cloudflare Worker on every Webflow publish (site publish for marketing; per-collection CMS publish for the rest).
